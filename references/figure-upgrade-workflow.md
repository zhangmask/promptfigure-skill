...
> 场景：用户扔来一篇论文 + 一堆已有图（多半是 MATLAB/Python/Origin 出的），要求「把图都变高级，再加几张流程图/机制图」。本文件覆盖：**① 需求分型 ② 澄清协议 ③ 两种执行模式 ④ 代码图数据溯源 ⑤ 追溯台账**。插图位定位与上下文提取见 `document-workflow.md`；PDF/WPS 格式、主动插图建议、参考论文风格库见 `proactive-upgrade.md`。
